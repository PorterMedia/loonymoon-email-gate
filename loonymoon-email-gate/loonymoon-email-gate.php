...
require_once LMEG_PLUGIN_DIR . 'includes/icons.php';
require_once LMEG_PLUGIN_DIR . 'includes/security.php';
require_once LMEG_PLUGIN_DIR . 'includes/charts.php';
require_once LMEG_PLUGIN_DIR . 'includes/email.php';
...
require_once LMEG_PLUGIN_DIR . 'includes/tiktok.php';
...
require_once LMEG_PLUGIN_DIR . 'includes/stage.php';
require_once LMEG_PLUGIN_DIR . 'includes/plan-brief.php';
require_once LMEG_PLUGIN_DIR . 'includes/plan-costs.php';
require_once LMEG_PLUGIN_DIR . 'includes/plan.php';
require_once LMEG_PLUGIN_DIR . 'includes/plan-print.php';
...
require_once LMEG_PLUGIN_DIR . 'includes/wallet.php';
require_once LMEG_PLUGIN_DIR . 'includes/presaves.php';
...
function lmeg_create_tables() {
    ...
    dbDelta("CREATE TABLE $seqsteps (
        ...
        subject VARCHAR(255) DEFAULT NULL,
        body_email LONGTEXT DEFAULT NULL,
        body_email_blocks LONGTEXT DEFAULT NULL,
        body_sms LONGTEXT DEFAULT NULL,
    ...
    dbDelta("CREATE TABLE $products (
        ...
        personalization VARCHAR(255) DEFAULT NULL,
        ...
        compare_at_cents INT UNSIGNED NOT NULL DEFAULT 0,
    ...
    dbDelta("CREATE TABLE $purchases (
        ...
        currency VARCHAR(3) NOT NULL DEFAULT 'USD',
        discount_code VARCHAR(60) DEFAULT NULL,
        ...
        ship_address TEXT,
        pickup_show VARCHAR(255) DEFAULT NULL,
        tracking VARCHAR(190) DEFAULT NULL,
...
function lmeg_default_settings() {
    return [
        ...
        'square_webhook_key'       => '',
        // Store (Beta) sale notifications to the artist/admin
        'store_notify'             => 1,
        'store_notify_email'       => '',
        // Store (Beta) tax/VAT — percent applied to the (discounted) item subtotal; 0 = off
        'store_tax_rate'           => 0,
        'store_tax_label'          => 'Tax',
        // Store (Beta) demo checkout — orders complete WITHOUT payment (testing)
        'store_demo'               => 0,
        // Store (Beta) automatic abandoned-cart reminder
        'store_cart_nudge'         => 0,
        'store_cart_nudge_hours'   => 1,
        'store_cart_nudge_pct'     => 0,   // optional % comeback discount in the reminder; 0 = off
        // Store (Beta) announcement banner
        'store_banner_text'        => '',
        'store_banner_link'        => '',
        // Store (Beta) weekly low-stock digest to the artist/admin
        'store_lowstock_digest'    => 0,
        // Store (Beta) auto-email the waitlist the moment a sold-out product is restocked
        'store_waitlist_auto'      => 1,
        ...
        'store_ship_us'            => 0,
        'store_ship_intl'          => 0,
        'store_ship_free_over'     => 0,   // free shipping when subtotal ≥ this (cents); 0 = off
        'store_ship_from'          => '',
        ...
        'ai_voice'                 => '',   // optional artist voice/tone notes for the AI composer
        ...
        'ig_account_id'            => '',
        'ig_verify_token'          => '',
        'ig_request_insights'      => 0,   // request instagram_manage_insights on connect (only after it's enabled on the Meta app)
        'tiktok_client_key'        => '',
        'tiktok_client_secret'     => '',
        'ig_story_reply'           => 'omg thank you for sharing!! 🙌 it means the world. mind if we repost it? reply YES and we\'ll tag you 💜',
...
function lmeg_render_merge_tags($text, $sub) {
    ...
    // {contest_link} still resolves to the newest open one). Always replace it,
    // even without a real subscriber (e.g. a test send to a non-subscriber) —
    // otherwise the literal "{contest_link:2}" ships in the email and an email
    // click-tracker (Brevo) wraps that non-URL and produces a broken link.
    if (strpos($text, '{contest_link:') !== false && function_exists('lmeg_contest_enter_url')) {
        $text = preg_replace_callback('/\{contest_link:(\d+)\}/', function ($m) use ($sub) {
            $sid = !empty($sub->id) ? (int) $sub->id : 0;
            return $sid ? lmeg_contest_enter_url((int) $m[1], $sid) : home_url('/');
        }, (string) $text);
    }
