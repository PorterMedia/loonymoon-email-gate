...
= 3.105.0 =
* New: Time on site + visit frequency per fan. On-site tracking now measures how long a signed-in fan actively spends on each page (a lightweight, visibility-aware timer that only counts while the tab is in front, capped at 30 min, reported as they leave). The central-brain export now exposes, per fan, their total time on site, number of visits (sessions split on a 30-minute gap) and average visit length — alongside the page views and days-visited it already reported — plus site-wide "page views (30d)" and "average time on site". Read-only analytics that no generic plugin ties to a known fan; nothing about the store, checkout or emails changes. Adds a dwell_ms column to the events table.
...
= 3.99.4 =
* Accessibility: every field on the checkout page now has a proper label for screen readers and password-manager autofill. The shipping address inputs, discount code, country, email, order note and pick-up fields previously relied on placeholder text alone (which disappears as you type and isn't announced by assistive tech). Purely additive — no visual change, and order totals/fulfilment are untouched.
...
= 3.99.1 =
* Performance: the Shop Revenue page counted its total number of orders twice per load (the same query ran once near the top and again for the "Recent orders" heading). It now counts once and reuses the result. Completes the admin performance pass.
...
= 3.93.0 =
* Performance: Faster Orders admin (round 2). The Orders page recomputed its all-time headline numbers (revenue, order count, customer count, top customers) from scratch on every load — including a distinct-order count that can't use an index. Those now come from one 3-minute cache and refresh instantly when you refund an order; the "to ship" number reuses the existing cached count. The filtered/searched order list itself is still live. No schema change.
...
= 3.84.0 =
* Store (Beta): Readability — darker "remove" / "back" links. The cart drawer's "remove" links (and the "Back to site / Keep shopping" link on the order pages) were a pale grey that sat right at the edge of legibility on white. They're now a darker grey that comfortably passes accessibility contrast. Came out of a contrast audit against a dark shop accent colour — which the rest of the store handled correctly (buttons, cart button, badges and chips all stayed readable). No schema change.
...
= 3.79.0 =
* Store (Beta): Native share button on product pages. The product page already had Copy-link / X / Facebook / WhatsApp buttons; it now leads with a "Share" pill that opens the phone's native share sheet — Instagram, Messages, AirDrop, whatever the fan uses — pre-filled with the product name and link. It only appears on devices that support it (progressive enhancement), so desktop is unchanged and still shows the existing buttons. Lets a fan share a product they love before they even buy. No schema change.
...
= 3.70.0 =
* Store (Beta): Countdown on the announcement banner. Give your storefront banner an optional end time (Store → Settings → Announcement banner) and it shows a live "ends in 2d 4h" countdown — then disappears on its own once the time passes. Perfect urgency for a limited drop or a sale. No schema change.

= 3.69.0 =
* Store (Beta): Product rich results (SEO). Each product page now includes Product structured data (JSON-LD) — name, image, price, currency and live availability (in stock / pre-order / sold out) — so Google can show your merch with price and a photo right in search results. Complements the existing social-share tags. No setup, no schema change.

= 3.68.0 =
* Store (Beta): Shareable discount links. Add ?code=YOURCODE to any link to your shop — e.g. yoursite.com/shop/?code=TOUR20 — and the discount is remembered as the fan browses and auto-applies at checkout (still validated server-side). The cart shows a "Code applied" chip they can remove. Perfect for putting a working promo straight in an email, an Instagram bio, or a QR code. No schema change.

= 3.67.0 =
* Store (Beta): "You might also like" in the cart. When there are items in the cart, the drawer now suggests a few of your best-selling products that aren't already in it, each with a one-tap Add — a gentle nudge to grow the order. Only offers simple, in-stock items (no size/name-your-price picks) so adding is instant, and suggestions update as items go in. No schema change.

= 3.66.0 =
* Store (Beta): Custom product badges. Add a short badge to any product — e.g. "Limited edition", "Signed", "Tour exclusive", "Last chance" — and it shows as a bold pill in your accent colour on the product card and page. Set it in the product editor (Badge field); it also imports/exports via the product CSV (new Badge column). Adds a small database column on update.
